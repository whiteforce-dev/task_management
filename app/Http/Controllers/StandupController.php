...
class StandupController extends Controller
{
    public function dailyStandup(){
        ...
        if(empty($standup)){
            $auth_user_tasks = Taskmaster::whereRaw("FIND_IN_SET(".Auth::user()->id.", alloted_to)")->where('status','!=',3)->select('id','task_name','priority','task_code')->get();
            ...
            $auth_user_tasks = Taskmaster::whereIn('id',explode(',',$standup->checkin))->select('id','task_name','priority','task_code')->get();
