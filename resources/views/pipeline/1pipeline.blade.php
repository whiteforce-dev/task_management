...
@section('content')
                ...
                <div class="new-box book">
                    ...
                    @foreach ($holdingtasks as $holdingtask)
                        <div class="three first-bb" style="position: relative;">
                            ...
                            <p><span>Start - </span>{{ \Carbon\Carbon::parse($holdingtask->start_date)->format('d-m-Y') }}
                            </p>
                            <p><span>Deadline -
                                </span>{{ \Carbon\Carbon::parse($holdingtask->deadline_date)->format('d-m-Y') }}</p>
                ...
                <div class="new-box onhold">
                    ...
                    @foreach ($progresstasks as $progresstask)
                        <div class="three third-dd" style="position:relative;">
                            ...
                            <p><span>Task - </span>{{ $taskname }}</p>
                            <p><span>Start - </span>{{ \Carbon\Carbon::parse($progresstask->start_date)->format('d-m-Y') }}
                            </p>
                            <p><span>Deadline -
                                </span>{{ \Carbon\Carbon::parse($progresstask->deadline_date)->format('d-m-Y') }}</p>
                    ...
                    @foreach ($completedtasks as $completedtask)
                        <div class="three four-ee" style="position: relative;">
                            ...
                            @if ($completedtask->status == '3')
                                <p><span>End Date -
                                    </span>{{ \Carbon\Carbon::parse($completedtask->end_date)->format('d-m-Y') }}</p>
                            @else
                                <p><span>Deadline -
                                    </span>{{ \Carbon\Carbon::parse($completedtask->deadline_date)->format('d-m-Y') }}</p>
                            @endif
    ...
    <script>
        function sendemail(task_id) {
            $.get("{{ url('sendtask-email') }}" + '/' + task_id, {}, function(response) {
                // location.reload()
                $('#response').html(response);
            });
        };
